...
from simulations.VarianceCovariance import simulate_varcov
import time
import json
import config
import logging
...
_SIM = "VarianceCovariance"
logging.basicConfig(filename='./log/risk_forecasts.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')


def parse_args():
    parser = argparse.ArgumentParser(description=f"Calculate the {_SIM} risk forecasts (VaR/ES). Writes the results directly into the simulation folder.")
...
def main():
    ...
    print(json.dumps(_params, indent=4)) # Only show params for this run, not the merged params
    ...
    print(f"Calculating {args.simulation} VaR & ES...")
