...
def main():
    ...
    _params = {"portfolio":args.portfolio,
          "volatility": {
            ...
            "controls": VineCopula.get_controls(controls).update({"tree_criterion":"tailtau"})
        }
    ...
    # Build the callable
    calc_fn = partial(simulate_vc_tailtau, 
                    controls=controls,
                    margin_dist=args.margin_distribution,
                    n_samples=args.n,
                    alpha=args.alpha,
                    tau_quantile=params.controls.tau_quantile if "tau_quantile" in params.controls else 0.25,
                    tau_tails=params.controls.tau_tails if "tau_tails" in params.controls else "both"
                    )
