...
from models import AdjustedReturn, HistoricalSimulation
import time
import json
import config
import sys
...
match args.risk_metric:
    case "VaR":
        ...
        # Concurrently -> Overhead! Linear calculation efficient for small portfolios
        if args.parallel:
            print("Calculating concurrently...")
            with ProcessPoolExecutor() as p:
                results = list(tqdm(p.map(partial(HistoricalSimulation.value_at_risk,alpha=params.simulation.alpha),adj_r_windows), total=len(adj_r_windows)))

        # Linear
        else:
            print("Calculating serial...")
            results = list(tqdm(map(partial(HistoricalSimulation.value_at_risk,alpha=params.simulation.alpha),adj_r_windows), total=len(adj_r_windows)))
    
    case "ES":
        if args.parallel:
            print("Calculating concurrently...")
            with ProcessPoolExecutor() as p:
                results = list(tqdm(p.map(partial(HistoricalSimulation.expected_shortfall,alpha=params.simulation.alpha),adj_r_windows), total=len(adj_r_windows)))
        else:
            print("Calculating serial...")
            results = list(tqdm(map(partial(HistoricalSimulation.expected_shortfall,alpha=params.simulation.alpha),adj_r_windows), total=len(adj_r_windows)))
